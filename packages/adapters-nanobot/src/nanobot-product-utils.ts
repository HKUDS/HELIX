...
import type { NanobotRegistrySnapshot, NanobotSurfaceHarness } from "./nanobot-product-types"

export function currentNanobotCwd(harness: NanobotSurfaceHarness): string {
  return String(harness.hooks.services.get("cwd") ?? process.cwd())
}

export function nanobotRegistrySnapshot(hooks: LegoHookHost): NanobotRegistrySnapshot {
  return {
...
export function nanobotMessageText(message: LegoMessage): string {
...
export async function readJSONBody<T>(request: IncomingMessage, maxBodyBytes: number): Promise<T> {
  ...
  for await (const chunk of request) {
    ...
    if (totalBytes > maxBodyBytes) throw new NanobotHTTPError(413, "Request body is too large.")
    chunks.push(buffer)
  }
  ...
  try {
    ...
    throw new NanobotHTTPError(400, "Expected a JSON request body.")
  }
}
...
export class NanobotHTTPError extends Error {
