...
import type { PiRegistrySnapshot, PiSurfaceHarness } from "./pi-product-types"

export function currentPiCwd(harness: PiSurfaceHarness): string {
  return String(harness.hooks.services.get("cwd") ?? process.cwd())
}

export function piRegistrySnapshot(hooks: LegoHookHost): PiRegistrySnapshot {
  return {
...
export function piMessageText(message: LegoMessage): string {
...
export async function readJSONBody<T>(request: IncomingMessage, maxBodyBytes: number): Promise<T> {
  ...
  for await (const chunk of request) {
    ...
    if (totalBytes > maxBodyBytes) throw new PiHTTPError(413, "Request body is too large.")
    chunks.push(buffer)
  }
  ...
  try {
    ...
    throw new PiHTTPError(400, "Expected a JSON request body.")
  }
}
